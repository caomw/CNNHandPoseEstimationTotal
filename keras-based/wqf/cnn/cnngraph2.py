...
ball=Graph()
ball.add_input(name='data96',ndim=4)
ball.add_node(BallModel(prenet=graph,batchsize=BATCHSIZE),name='ballxyz',input='data96')
print ball.get_output()
xy=Graph()
xy.add_input(name='data96',ndim=4)
xy.add_node(ConvertToXY(prenet=ball,batchsize=BATCHSIZE,height=HEIGHT,width=WIDTH,ballnum=BALLNUM),name='ballxy',input='data96')

main=Graph()
main.add_input(name='data96',ndim=4)
main.add_node(GaussianModel(prenet=xy,height=HEIGHT,width=WIDTH,ballnum=BALLNUM,sigma=SIGMA,batchsize=BATCHSIZE),name='gaussian',input='data96')
"""
...
main.compile(optimizer=sgd,loss={'final':'mse'})
