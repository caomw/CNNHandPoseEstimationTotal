...
#include<eigen/dense>  
#define eps 1e-6
#define oridir "D:\\CNN\\lstmheatmapjoint10\\lstmheatmapjoint10\\lstmheatmapjoint10\\lstm.txt"
#define st 1
#define en 8248
#define N 484
#define Height 22
...
void showjointonpicture()
{
	for (int i = st; i <= en; i++)
	{
		...
		waitKey(1);
		char savename[111];
		sprintf(savename, "%s%d%s", "J:\\cnnhandtotal\\lstmheatmap\\10\\lstm\\", i, ".png");
...
{
	ori();
	FILE *fout = fopen("D:\\CNN\\lstmheatmapjoint10\\lstmheatmapjoint10\\lstmheatmapjoint10\\lstmout.txt", "w");
	for (int i = st; i <= en; i++)
	{
		fprintf(fout, "%.2f %.2f\n", resx[i], resy[i]);
	}
