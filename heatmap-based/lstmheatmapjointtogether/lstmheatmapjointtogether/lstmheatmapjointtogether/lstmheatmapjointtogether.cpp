...
void ori(int mode)
{
	...
	for (int i = st; i <= en; i++)
	{
		...
		init(z);
		getcenterpoint();
		resx[i] = xx0-(mode==1); resy[i] = yy0-(mode==1);
		if (abs(maxz) < eps) { resx[i] = resx[i - 1]; resy[i] = resy[i - 1]; }
	}
}
void showjointonpicture(int joint,int mode)
{
	for (int i = st; i <= en; i++)
	{
		char s_img[111];
		cout << "Current Image Id:" << i << " Joint: " << joint << " mode: "<<folder[mode]<<"\n";
		...
		waitKey(1);
		char savename[111];		
		sprintf(savename, "%s%d%s%s%s%d%s", "J:\\cnnhandtotal\\lstmheatmap\\",joint,"\\",folder[mode],"\\", i, ".png");
...
int main()
{
	for (int joint = 2; joint < 14; joint++)
	{
		if (joint == 8 || joint == 10) continue;
		for (int mode = 0; mode < 3;mode++) //lstm ori target
		{
			sprintf(oridir, "%s%s%d%s", "D:\\CNN\\lstmheatmapjointtogether\\lstmheatmapjointtogether\\lstmheatmapjointtogether\\",folder[mode], joint, ".txt");
			ori(mode);
			char foutname[111];
			sprintf(foutname, "%s%s%d%s", "D:\\CNN\\lstmheatmapjointtogether\\lstmheatmapjointtogether\\lstmheatmapjointtogether\\", folder[mode], joint,"out.txt");
