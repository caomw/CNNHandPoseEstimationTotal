...
#include <highgui.h>
#include<opencv2/opencv.hpp>
#define priorprefix "I:\\libmodel\\deeppriordata\\"
#define datasetdir "I:\\libmodel\\deeppriordata\\"
#define fileprefix "I:\\libmodel\\deeppriorh5\\"
#define DIML1 24252
#define DIML2 93
#define SIZE 128
using namespace std;
...
void readgtjoint()
{
	...
	for (int i = 0; i < 72756; i++)
	{
		for (int j = 0; j < 93; j++)
		{
			fscanf(fjoint, "%f", &joint[seq[i]][j]); //seq 0-72756  actually 1-72757
		}
	}
	for (int i = 0; i < 93; i++)
	{
...
void genexpand()
{
	for (int i = 0; i < 72756 / DIML1; i++)
	{
		...
		en = (i + 1)*DIML1 - 1;
		nowcnt = 0;
		sprintf(file, "%s%s%d%s", fileprefix,"train_", i + 1, ".h5");
		localization();
		genHDF5();
	}
}
