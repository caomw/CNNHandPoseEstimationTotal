...
#define outputtestdir "J:\\cnnhandtotal\\cnntraindata\\nyutest\\"
	...
	for (int i = 2; i <= 8252; i++)
	{
		...
		for (int h = minh; h <= maxh; h++)
		{
			for (int w = minw; w <= maxw; w++)
			{
				...
				middle.at<uchar>(h - minh, w - minw) = (float(depth - mind) / (maxd - mind))*255.0;
		...
		char savename[111];
		sprintf(savename, "%s%d%s", outputtestdir, i, ".png");
		imwrite(savename, middle);
		//imshow("", out);
		//waitKey(1);
	}
