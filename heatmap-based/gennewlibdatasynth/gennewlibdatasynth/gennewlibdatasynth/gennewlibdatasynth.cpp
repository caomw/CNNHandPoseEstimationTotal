...
Mat changesize(Mat src, int size)
{
	...
	for (int row = 0; row < src.rows; row++)
	{
		...
		{
			//cout << row << " " << col << "\n";
			if (row > (int)((1 - size / 100.0) / 2 * src.rows ) && row < src.rows - del &&
				col > (int)((1 - size / 100.0) / 2 * src.rows ) && col < src.cols - del)
			{
				int trow = row - del;
				int tcol = col - del;
	...
	for (int i = 0; i < 72756; i++)
	{
		...
		for (int h = minh; h <= maxh; h++)
		{
			for (int w = minw; w <= maxw; w++)
			{
				...
				int depth = skin.at<Vec3b>(h, w)[1] * 256 + skin.at<Vec3b>(h, w)[0];
				mind = min(mind, depth);
				maxd = max(maxd, depth);
		...
		for (int h = minh; h <= maxh; h++)
		{
			for (int w = minw; w <= maxw; w++)
			{
				...
				middle.at<uchar>(h - minh, w - minw) =(int)((double(depth - mind) / double(maxd - mind))*255.0);
		...
		/*
		char srcname[111];
		sprintf(srcname, "%s%d%s", "J:\\cnnhandtotal\\cnntraindata\\size85\\", i + 1, ".png");
		Mat src = imread(srcname);
		Mat middle = changesize(src, 100.0/85.0*100.0);
		imshow("", middle);
		waitKey(0);*/
		...
		char savename[111];
		sprintf(savename, "%s%d%s", outputsynthdepdir, i + 1, ".png");
