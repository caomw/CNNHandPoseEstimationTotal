// genexpand128.cpp : 定义控制台应用程序的入口点。
//
...
#include <highgui.h>
#include<opencv2/opencv.hpp>
#define priorprefix "J:\\cnnhandtotal\\cubedata\\imgrealdep\\"
#define datasetdir "J:\\cnnhandtotal\\cubedata\\imgrealdep\\"
#define fileprefix "I:\\libmodel\\expandcube\\"
#define DIML1 24252
#define DIML2 93
#define SIZE 128
using namespace std;
using namespace cv;
float depthdata1[24252][1][128][128];

float joint[72758][93];
float data[DIML1][1][SIZE][SIZE];
float label[DIML1][DIML2];
int seq[72758];
int seqexpand[230394];
int st, en, nowcnt;
char file[111];
void init()
{
...
{
cout << i << "\n";
Mat z = Mat::zeros(128, 128, CV_8UC1);
for (int j = 0; j < 128; j++)
{
for (int k = 0; k < 128; k++)
{
...
void readexpandseq()
{
	FILE *fexpand = fopen("D:\\CNN\\genlibmodeltrainHDF5\\seqexpand.txt", "r");
	for (int i = 0; i < 230394; i++)
	{
...
void localization()
{
	...
	for (int i = st; i <= en; i++)
	{
		...
		Mat rgb = Mat::zeros(128, 128, CV_8UC3);
		...
		for (int j = 0; j < 31; j++)
		{
			int gtu = (int)((label[nowcnt - 1][j * 3] + 1.0) * 128.0 / 2);
			int gtv = (int)((-label[nowcnt - 1][j * 3 + 1] + 1.0)*128.0 / 2.0);
			//cout << gtu << " " << gtv << "\n";
